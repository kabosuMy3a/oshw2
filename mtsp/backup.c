...
void _travel(int idx, int * arr, int* prefix_arr, int *search_count, int *path, int *used, int length){
		...
		*search_count += 1;
		...
		for( i = 0; i< 11 ; i++){
			if(used[i] == 0){
				path[idx] = arr[i+*PS];
				used[i] = 1;
				_travel(idx+1,arr,prefix_arr, search_count, path, used, length);
				used[i] =0;
			}		
		}
	}
...
void travel(int start, int *arr, int * prefix_arr, int * search_count){ 
	int path[11] ;
	int used[11] ;
	int length = 0 ;
	...
	path[0] = arr[*PS+start] ;
	used[start] = 1;
	_travel(1,arr,prefix_arr, search_count, path, used, length);
	used[start] = 0;
...
void* customer_routine(void * OFFSET){
	...
	for(int i = 0 ; i < 11 ; i++){
		travel(i, arr,prefix_arr, &search_count);
	}
...
void start(){
	...
	/*while(1){
		pthread_t temp ;
		if(current_thread_num < *PB){
			pthread_create(&temp, NULL, customer_routine, 0x0);
			pthread_mutex_lock(&m2);
				current_thread_num += 1;
			pthread_mutex_unlock(&m2);
		}
